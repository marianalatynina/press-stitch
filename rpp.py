...
class RenPyLabelCall:
    ...
    def __repr__(self):
        # type: () -> str
        return f"{self.label}, {str(self.vars)}"
...
class RenPyBlock(RenPyObject):
    ...
    def __repr__(self):
        # type: () -> str
        return f"blk({str(self.startLine)},{str(self.indent)}) processing line " + str(
            self.lineNum
        )
...
class RenPyIf(RenPyObject):
    ...
    def __repr__(self):
        return f"if({str(self.startLine)},{str(self.indent)}) processing line " + str(
            self.lineNum
        )
...
class RenPyThread:
    def __init__(self, l, v, s):
        # type: (str, dict[str,str], list[RenPyObject]) -> None
        self.label = l
        self.vars = v
        self.stack = s
...
class RenPyFileCiel(RenPyFile):
    ...
    def addMutators(self, charName):
        # type: (str) -> str
        return "ciel hair headband" if charName == "ciel" else charName
...
class RenPyFileHideDevice(RenPyFile):
    ...
    def readFile(self, fn):
        ...
        self.lines[
            196
        ] = '            call screen pending_001(path_name = "Trio Swap", author_note = "This path is under development, check back soon!")\n'
    ...
    def addMutators(self, charName):
        # type: (str) -> str
        return f'{charName}d' if charName in ["main", "mika"] else charName
...
class RenPyFileEliza(RenPyFile):
    ...
    def addMutators(self, charName):
        # type: (str) -> str
        return "cield" if charName == "ciel" else charName
...
class RenPyFileNick(RenPyFile):
    def __init__(self, b, c, v6):
        ...
        self.charMap = c
        self.v6Map = v6
        self.charFlip = ["ashley", "candice", "main", "maind", "melina", "nurse"]
        self.trackVis = True
        self.flipAll = True
