...
# -----------------------------------------------------------------------------
def doCrop(srcDir):
    dstDir = f"{srcDir}Cropped"
    doMakeDir(dstDir)
    for pic in os.listdir(srcDir):
        if pic.endswith(".png"):
            srcFile = os.path.join(srcDir, pic)
            dstFile = os.path.join(dstDir, f"{pic}.pnm")
            print(f"Cropping {srcFile}")
            cmd = [
                "convert",
                "-size",
                "1024x1024",
...
def printArrayEntry(src, dst, comment):
    ...
    print(f"  {first} {second} # Auto: {comment}")
            ...
            if asArray:
                ...
                if chksum in idx:
                    ...
                    if filename6 == pic:
                        ...
                        printGreen(
                            leftAlign(pic, fileWidth) + ": Renamed to " + filename6
                        )
                else:
                    if os.path.exists(os.path.join(srcDir6Crop, pic)):
