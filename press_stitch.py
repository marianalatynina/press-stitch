...
from maps import (
    backgrounds_map,
    character_map_35_ashley,
    character_map_35_candice,
    character_map_35_chris,
    character_map_35_ciel,
    character_map_35_cindy,
    character_map_35_eliza,
    character_map_35_jenna,
    character_map_35_karyn,
    character_map_35_main,
    character_map_35_martha,
    character_map_35_melina,
    character_map_35_michelle,
    character_map_35_mika,
    character_map_35_mother,
    character_map_35_nick,
    character_map_35_nurse,
    character_map_35_vanessa,
    character_map_45_alma,
    character_map_45_amber,
    character_map_45_anna,
    character_map_45_april,
    character_map_45_candice,
    character_map_45_chris,
    character_map_45_ciel,
    character_map_45_cindy,
    character_map_45_donald,
    character_map_45_eliza,
    character_map_45_erin,
    character_map_45_ermach,
    character_map_45_hillary,
    character_map_45_jenna,
    character_map_45_jennifer,
    character_map_45_jillian,
    character_map_45_karyn,
    character_map_45_kayla,
    character_map_45_main,
    character_map_45_martha,
    character_map_45_melina,
    character_map_45_michelle,
    character_map_45_mika,
    character_map_45_mother,
    character_map_45_nelson,
    character_map_45_nick,
    character_map_45_nurse,
    character_map_45_sean,
    character_map_45_vanessa,
    character_map_45_waitress,
    character_map_56_eliza,
    character_map_56_main,
)
...
def verifySingleFile(filename, desiredHash):
    ...
    if actualHash != desiredHash:
        ...
        print(f"Desired MD5: {desiredHash}")
        print(f"Actual MD5 : {actualHash}")
        return False
...
def checkFile(dirname, checksum):
    ...
    if os.path.isdir(pathlib.Path(dirname)):
        print(f"Directory {dirname} exists, ZIP extract skipped")
        return True
...
def doCopyFile(srcPath, dstPath, filename):
    ...
    print(f"Copying file {srcFile} into {dstPath}")
...
def expandNumberField(s):
    # type: (str) -> str
    return s if not (isNumberField(s)) else s.zfill(3)
    ...
    if fields[1] == "=":
        if fields[2].startswith("renpy.random.choice(["):
            ...
            for choice in choiceList:
                ...
                newThread.stack[-1].lineNum = lineNum + 1
                # Start on next line
            ...
            if ".display" in pyVar:
                if "hillary cas" in pyVal and "hillary casual" not in pyVal:
                    pyVal = "hillary"
                    rpFile.lines[lineNum] = rpFile.lines[lineNum].replace(
                        "hillary cas", "hillary casual"
                    )
                if "hillary school" in pyVal:
                    pyVal = "hillary"
                    rpFile.lines[lineNum] = rpFile.lines[lineNum].replace(
                        "hillary school", "hillary"
                    )
        ...
        flagError(
            rpFile,
            lineNum,
            "Unsupported operator '" + fields[1] + "', line is: " + line,
        )
...
def processBlockStep(rpFile, thread):
    ...
    if not (rpFile.indentIsGood(i, indent)) and lineType != rpp.LineType.LABEL:
        thread.stack.pop()
        return
    ...
    if lineType == rpp.LineType.IF:
        thread.stack.append(rpp.RenPyIf(i, indent))
    ...
    if lineType == rpp.LineType.HIDE:
        ...
        if rpFile.trackVis and person != "bg":
            thread.vars[f"_visible_{person}"] = "0"
        ...
        if not (rpFile.labelIsAcceptable(label)):
            ...
            thread.stack = []
            return
        if label not in rpFile.labelList:
            print(f"External jump: {label}")
            thread.stack = []
            # Kill this thread, it jumped out of the file
            return
        addLabelCall(rpFile, label, thread)
        thread.stack = []
        # Kill this thread, it jumped
        return
...
def processMenuStep(rpFile, thread, lineNum):
    ...
    while (lineNum < rpFile.numLines) and rpFile.indentIsGood(lineNum, indent):
        if getIndentOf(line) == indent:
            menuItem = line.strip("\n").strip("\r").strip()
            if (":" in menuItem) and not (
                (menuItem[0] == "#") or menuItem.startswith('"{s}')
            ):
...
def processShow(rpFile, thread, lineNum):
    ...
    if fields[1] == "bg":
        ...
        if not (fields[2] in rpFile.backMap):
            return flagError(
                rpFile,
                lineNum,
                "Background " + fields[2] + " has no mapping table entry",
            )
            return line
        ...
        if newbg == "":
            return flagError(
                rpFile,
                lineNum,
                "Background '" + fields[2] + "' exists but has no mapping",
            )
    ...
    if characterDoRemap[fields[1]]:
        ...
        if charName in personDispVars:
            ...
            return flagError(
                rpFile,
                lineNum,
                "Missing Python display variable for character '" + charName + "'",
            )
        swappedFields = swappedCharName.split()
        swappedCharName = swappedFields[0]
        if (
            (swappedCharName == "donald")
            and (len(swappedFields) > 1)
            and (swappedFields[1] == "girl")
        ):
            donaldGirl = True
    ...
    while i < len(fields):
        ...
        if filenameMode:
            ...
            if baseMode:
                if not (field == "full") and not (
                    (charName == "hillary") and (fields[i] == "school")
                ):
                    base = base + " " + fields[i]
            else:
    ...
    if (exFile == (swappedCharName + "_ex")) or (
        exFile == (swappedCharName + "_ex_full")
    ):
    ...
    if swappedCharName in rpFile.charMap:
        ...
        return flagError(
            rpFile,
            lineNum,
            "No mapping entry for character '"
            + swappedCharName
            + "', source file '"
            + exFile
            + "'",
    ...
    if not (hasMapped):
        ...
        return flagError(
            rpFile,
            lineNum,
            "Mapping failed, source file '"
            + exFile
            + "' not found. Line being processed is: "
            + str(fields),
        )
    ...
    if mappedFile == "":
        return flagError(
            ...
            "Mapping failed, source file '"
            + exFile
            + "' exists but has no mapping. Line being processed is: "
            + str(fields),
        )
    ...
    if not (mappedFields[0] == swappedCharName):
        return flagError(
            rpFile,
            lineNum,
            "Mapped to a different character! Source is '"
            + exFile
            + "', map is '"
            + mappedFile
            + "'",
    ...
    if not (mappedFields[1] == "ex"):
        return flagError(
            rpFile,
            lineNum,
            "Mapping is not to an expression graphic! Source is '"
            + exFile
            + "', map is '"
            + mappedFile
            + "'",
...
def processNextThread(rpFile):
    ...
    while len(thread.stack) > 0:
        ...
        if obj.objType == "Block":
            ...
            print(f"Unhandled object type: {obj.objType}")
            sys.exit(1)
...
def iterateLabelCalls(rpFile):
    ...
    while (len(labelCalls) > 0) or (len(threads) > 0):
        print(f"---------- Depth {iterations} ----------")
        print(f"Label calls: {len(labelCalls)}")

        # Process label calls
        while len(labelCalls) > 0:
            labelCall = labelCalls.pop()
            if labelCall not in labelCalls:
                processLabelCall(rpFile, labelCall.label, labelCall.vars)
            else:
                duplicates += 1

        # Process threads
        print(f"Paths: {len(threads)}")
        while len(threads) > 0:
...
def verifySingleCharacterMapping(charName, desc, mapTable, charactersDir):
    ...
    filesLower = [filename.lower() for filename in os.listdir(imageDir)]
    for k in mapTable:
        if mapTable[k] != "":
            if f"{mapTable[k]}.png" not in filesLower:
                printRed(
                    "ERROR: Key '"
                    + k
                    + "', value '"
                    + mapTable[k]
                    + "': Map destination file not found"
                )
                return False
...
def verifyCharacterMapping(desc, mapTable, extractedDir):
    ...
    return all(
        (verifySingleCharacterMapping(charName, desc, mapTable[charName], imageDir))
...
def main(argv):
    ...
    dayzero.lines.insert(
        2848, (" " * 28) + '"Maybe I was too quick to reject Eliza...":\n'
    )
    ...
    if doNick:
        ...
        hideDevicePath = rpp.RenPyFileHideDevice(
            backgrounds_map.backgroundMap35, characterImageMap35, v6map
        )
    ...
    if doEliza:
        ...
        elizaPath = rpp.RenPyFileEliza(
            backgrounds_map.backgroundMap45, characterImageMap45, v6map
        )
        ...
        if doScan:
            ...
            pyVariables["timer_value"] = 0
            # Less than 30
            addLabelCall(
                elizaPath, "eliza", rpp.RenPyThread("", pyVariables.copy(), [])
            )
            pyVariables["timer_value"] = 60
            # Greater than 30
            addLabelCall(
                elizaPath, "eliza", rpp.RenPyThread("", pyVariables.copy(), [])
            )
    ...
    effectsFile.lines[
        495
    ] = '    timer 1 repeat True action SetVariable("timer_value", timer_value + 1)\n'
    ...
    screensFile.lines[
        190
    ] = '        text "{=bio_font}{size=+10}{color=32eeff}Press-Switch V:0.5c-merged{/color}{/size}{/bio_font}":\n'
